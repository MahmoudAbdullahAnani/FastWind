      ...
      {showMenu && (
        <>
          <div className="absolute inset-x-0 top-14 px-2 z-[9]">
            ...
            <div className={`bg-white rounded-t-md p-2 flex items-center justify-between`}>
              <HeaderSearch />
              <div className="flex justify-between gap-2 items-center">
                <BtnSocial href="https://github.com/MahmoudAbdullahAnani/FastWind"
                  rel="noreferrer"
                  target="_blank"
                  className="inline-flex items-center gap-[3px] text-gray-900 hover:opacity-75" icon={<IconGithub stargazers_count={window.sessionStorage.getItem('stargazers_count')} showK={true} />} title={<span className={`sr-only`}> GitHub </span>}
                />
            ...
            <MenuLinks
              menuLinks={menuLinks}
              navClass="bg-white border p-4 border-gray-200 shadow-lg rounded-b-md"
              ulClass="space-y-4"
            />
          </div>
